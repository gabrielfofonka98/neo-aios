...
- **Integração NEO-AIOS:** Gera agentes no formato simplificado com scope enforcement

---

## 🆕 Integração NEO-AIOS

O Squad Creator agora suporta criação de agentes no formato **NEO-AIOS**, o framework de agentes com:

- **Scope Enforcement:** Apenas Gage (DevOps) pode fazer `git_push`
- **Hierarquia Big Tech:** c-level → vp → director → manager → ic
- **Comunicação PT-BR:** Toda comunicação em português, código em inglês
- **Estrutura Simplificada:** Formato mais direto que o legacy
...
# Criar agente no formato NEO-AIOS
*create-neo-aios-agent

# Seguir workflow interativo
```
...
- `commands:` - Comandos disponíveis (com *)

Ver `templates/neo-aios-agent-tmpl.md` para o template completo.
