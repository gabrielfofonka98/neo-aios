...
commands:
  ...
  - check-prd {prd_file}: Analyze PRD compatibility with system
...
dependencies:
  ...
  coderabbit_integration:
    ...
    workflow: |
      ...
      1. Run: wsl bash -c 'cd  && ~/.local/bin/coderabbit --prompt-only -t uncommitted' (for ongoing work)
      2. Or: wsl bash -c 'cd  && ~/.local/bin/coderabbit --prompt-only --base main' (for feature branches)
...
- **@ux (Uma):** For frontend architecture and user flows
...
3. **Collaboration** → Coordinate with @data-engineer (database) and @ux (frontend)
