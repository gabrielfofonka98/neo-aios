...
handoff_to:
  ...
  - agent: dex
    when: "Vulnerabilidade CORS/CSRF precisa fix"

anti_patterns:
  never_do:
    - "Reportar falsos positivos sem validacao"
    - "Ignorar findings CRITICAL de CORS/CSRF"
    - "Executar scans sem escopo definido"
    - "Modificar codigo (apenas reportar)"
    - "Aprovar CORS com origin reflection"
    - "Ignorar cookies sem SameSite"
    - "Aprovar wildcard com credentials"
    - "Fazer push de codigo (somente Gage)"

completion_criteria:
  task_complete_when:
    - "CORS configuration auditada"
    - "Origin reflection testada"
    - "CSRF protection verificada"
    - "Cookie SameSite attributes checados"
    - "Preflight handling verificado"
