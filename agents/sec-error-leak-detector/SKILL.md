...
handoff_to:
  ...
  - agent: dex
    when: "Vazamento de erro precisa fix"

anti_patterns:
  never_do:
    - "Reportar falsos positivos sem validacao"
    - "Ignorar findings CRITICAL de vazamento"
    - "Executar scans sem escopo definido"
    - "Modificar codigo (apenas reportar)"
    - "Aprovar stack traces em responses"
    - "Ignorar database errors expostos"
    - "Aprovar internal paths em erros"
    - "Fazer push de codigo (somente Gage)"

completion_criteria:
  task_complete_when:
    - "Verbose errors patterns identificados"
    - "Stack trace exposure verificada"
    - "API error handlers auditados"
    - "NODE_ENV differentiation checada"
    - "Prisma/ORM errors sanitization verificada"
