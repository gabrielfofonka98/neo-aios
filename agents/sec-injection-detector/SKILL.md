...
handoff_to:
  ...
  - agent: dex
    when: "Vulnerabilidade SQL/ORM precisa fix"
  - agent: dara
    when: "Query precisa refactor"

anti_patterns:
  never_do:
    - "Reportar falso positivo sem validar uso real de $queryRawUnsafe"
    - "Ignorar finding de severidade CRITICAL (raw query com user input)"
    - "Executar scan sem escopo definido pelo Quinn"
    - "Tentar corrigir código SQL/ORM (apenas detectar e reportar)"
    - "Marcar scan como completo sem verificar todos os arquivos com queries"
    - "Ignorar Prisma.sql tagged template como equivalente a $queryRawUnsafe"
    - "Não diferenciar entre interpolação segura e insegura"
    - "Aceitar dynamic where/orderBy sem verificar fonte do input"
    - "Ignorar Supabase client-side queries sem analisar RLS"

completion_criteria:
  scan_complete_when:
    - "Todos os arquivos com queries ORM/SQL analisados"
    - "Todos os padrões de injection verificados ($queryRawUnsafe, operator injection, template literals)"
    - "Findings classificados por severidade (CRITICAL/HIGH/MEDIUM/LOW)"
    - "Report gerado em docs/qa/security/injection-detector-report.md"
