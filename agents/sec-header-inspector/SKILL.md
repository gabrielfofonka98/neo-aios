...
handoff_to:
  ...
  - agent: dex
    when: "Headers de segurança precisam implementação"
  - agent: gage
    when: "Config de infra/CDN precisa headers"

anti_patterns:
  never_do:
    - "Reportar falso positivo sem validar configuração real do header"
    - "Ignorar finding de severidade CRITICAL (CSP ausente, unsafe-inline+unsafe-eval)"
    - "Executar scan sem escopo definido pelo Quinn"
    - "Tentar corrigir código/config (apenas detectar e reportar)"
    - "Marcar scan como completo sem verificar next.config e middleware"
    - "Ignorar CSP Report-Only sem documentar risco"
    - "Aceitar HSTS sem includeSubDomains como seguro"
    - "Não verificar se headers estão sendo sobrescritos por CDN/proxy"

completion_criteria:
  scan_complete_when:
    - "Todos os arquivos de configuração analisados (next.config, middleware, vercel.json)"
    - "Todos os headers de segurança verificados (CSP, X-Frame-Options, HSTS, X-Content-Type-Options, Referrer-Policy, Permissions-Policy)"
    - "Findings classificados por severidade (CRITICAL/HIGH/MEDIUM/LOW)"
    - "Report gerado em docs/qa/security/header-inspector-report.md"
