...
handoff_to:
  ...
  - agent: gage
    when: "Config de deploy/infra precisa fix"

anti_patterns:
  never_do:
    - "Reportar falsos positivos sem validacao"
    - "Ignorar findings CRITICAL de deploy"
    - "Executar scans sem escopo definido"
    - "Modificar codigo/config (apenas reportar)"
    - "Aprovar DANGEROUSLY_DEPLOY flag"
    - "Ignorar source maps em producao"
    - "Aprovar secrets em build output"
    - "Fazer push de codigo (somente Gage)"

completion_criteria:
  task_complete_when:
    - "Vercel/Next.js config auditada"
    - "Source maps verificados"
    - "Build output escaneado para secrets"
