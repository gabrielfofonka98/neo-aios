...
handoff_to:
  ...
  - agent: dex
    when: "Validação Zod/types precisa implementação"

anti_patterns:
  never_do:
    - "Reportar falso positivo sem verificar contexto de uso do any type"
    - "Ignorar finding de severidade CRITICAL (API sem validação, strict:false)"
    - "Executar scan sem escopo definido pelo Quinn"
    - "Tentar implementar validação Zod (apenas detectar e reportar)"
    - "Marcar scan como completo sem verificar todos os API routes"
    - "Aceitar TypeScript types como suficiente para runtime safety"
    - "Ignorar any type como 'necessário em alguns casos'"
    - "Não verificar tsconfig strict mode"
    - "Aceitar Zod errors leaking details para client"

completion_criteria:
  scan_complete_when:
    - "Todos os API routes verificados para Zod validation"
    - "any type usage contado e localizado"
    - "tsconfig.json verificado para strict:true"
    - "Boundaries de input verificados (forms, external APIs)"
    - "Findings classificados por severidade (CRITICAL/HIGH/MEDIUM/LOW)"
    - "Report gerado em docs/qa/security/validation-enforcer-report.md"
