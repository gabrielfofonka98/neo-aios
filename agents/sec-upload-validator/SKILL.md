...
handoff_to:
  ...
  - agent: dex
    when: "Validação de upload precisa implementação"

anti_patterns:
  never_do:
    - "Reportar falso positivo sem verificar implementação real do upload"
    - "Ignorar finding de severidade CRITICAL (sem magic bytes, filename do user)"
    - "Executar scan sem escopo definido pelo Quinn"
    - "Tentar implementar validação (apenas detectar e reportar)"
    - "Marcar scan como completo sem verificar todos os upload handlers"
    - "Aceitar validação por extensão como suficiente"
    - "Ignorar user-provided filenames"
    - "Não verificar storage location (public/ é red flag)"
    - "Aceitar client-only size validation como suficiente"

completion_criteria:
  scan_complete_when:
    - "Todos os upload handlers identificados e analisados"
    - "Validação verificada: magic bytes, filename sanitization, size limits"
    - "Storage location verificado (deve ser fora de public/)"
    - "Findings classificados por severidade (CRITICAL/HIGH/MEDIUM/LOW)"
    - "Report gerado em docs/qa/security/upload-validator-report.md"
