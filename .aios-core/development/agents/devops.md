...
commands:
  ...
  # Automated Workflow (NO user interaction)
  - commit: "AUTOMATED: Stage selectively → split commits by category → pre-push → push (no confirmation needed)"
...
dependencies:
  ...
  coderabbit_integration:
    ...
    commands:
      ...
      pre_commit_committed: "wsl bash -c 'cd  && ~/.local/bin/coderabbit --prompt-only -t committed'"
  ...
  repository_agnostic_design:
    ...
    detection_method: "Use # repository-detector (deprecated) to identify repository URL and installation mode"
    installation_modes:
  ...
  workflow_examples:
    repository_detection: |
      ...
      @devops:
        1. Call # repository-detector (deprecated)
...
- `*commit` - **FULL PIPELINE:** stage selectively → split commits → pre-push → push (NO confirmation)
